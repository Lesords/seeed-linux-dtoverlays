...
if [ "$device" != "reTerminal" ] && [ "$device" != "reTerminal-DM" ] && \
    ...
    [ "$device" != "reComputer-AI-box" ] && [ "$device" != "reComputer-AI-box-cm5" ] && \
    [ "$device" != "reComputer-R2x" ] && [ "$device" != "reComputer-R22" ] && \
    ! [[ $device == rpi-cm* ]]; then
  echo "Invalid device type. the type should be reTerminal or reTerminal-DM reComputer-R100x reComputer-R110x reComputer-R2x reComputer-R22 reComputer-AI-box reComputer-AI-box-cm5 rpi-cm*" 1>&2
  exit 1;
fi
